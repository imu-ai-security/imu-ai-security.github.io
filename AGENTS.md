# Customizing this site with Codex

This file is both a **guide for you** and **context for [Codex](https://Codex.com/Codex)**.
Open this repo in Codex (or the Codex app with this folder attached) and work through the
...
> Tip: after each step, ask Codex to run `npm run build` to confirm the site still compiles.

---

## How this site is structured (so Codex gets it right)

- **Content is Markdown** in `src/content/{people,publications,figures,gallery}/`. Schemas are in
  `src/content.config.ts` — Codex should keep frontmatter valid against them.
...
Codex to crop each to a centered head-and-shoulders square (face on the upper third, a little
...
Codex to sanity-check each name against the person's public presence (an easy thing to misspell on a
...
surname instead (handy for a long Alumni list, so you never renumber when someone joins), ask Codex
...
Codex to add `citations:` manually.
...
**Codex-assisted profile finding — LinkedIn, Scholar, personal sites.** These aren't in publication
metadata, so ask Codex to look them up:
...
A wrong link on a public page is worse than none, so hold Codex to the "show evidence + confidence,
never guess" rule above and **verify before publishing**. For a big roster, ask Codex to fan the
...
**Codex: watch for template-level problems as you work, and when one comes up, proactively offer to
...
**If you have a concrete fix — open a pull request** (Codex orchestrates this):
...
# 2) Codex: in the fork, apply ONLY the template fix on a new branch, commit, push.
